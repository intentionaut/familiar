...
`--warm` for warmth and plain language at the writer's `Interview engagement`.
...
- **Draft copy:** `familiar push <piece>` and `familiar pull <piece>` (when the
  house turns draft copy on) run only when the writer asks. A pull is the
  writer's own edit coming home, so it counts as theirs; an agent never runs one
  unprompted and never passes `--force`. `draft.md` is still only ever changed by
  the writer.
...
- `scripts/question-check.py`: flags questions that break "Asking the writer"; run on prepared questions and decision gates; `--prepared` adds the three-prompt set checks, `--warm` adds the fireside warmth and plain-language checks
