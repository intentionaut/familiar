...
Run the Familiar **board**. Familiar lives at `${CLAUDE_PLUGIN_ROOT}`.
...
1. **First, catch up on anything that has been sent.** Run
   `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/pull-final.py`. It writes `final.md` for
...
2. You do not need to work out where the pieces are. `board.py` resolves them
   ...
   lines in a `.familiar` file, then `${CLAUDE_PLUGIN_ROOT}/pieces`. Pass `--pieces`
...
3. Run it, adding `--serve` when the writer wants to work on a piece or tidy up
   ...
   ```
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/board.py --open
   ```
...
`${CLAUDE_PLUGIN_ROOT}/prompts/draft.md`, "Comments from the board", and hand each
